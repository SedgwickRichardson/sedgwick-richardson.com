    ...
    $.each(Drupal.settings.getlocations, function (key, settings) {
      ...
      if ( $("#getlocations_map_canvas_" + key).is('div') ) {
        ...
        },
        {
          ...
          elementType: "labels", 
          stylers: [{
            visibility: (transit_show ? 'on' : 'off')
          }]
        },
        /*
         * HACKED BY KELVIN
        ...
        var mapOpts = {
          ...
          overviewMapControlOptions: {
            opened: (overview_opened ? true : false)
          },
        ...
        if (settings.trafficinfo) {
          ...
          $("#getlocations_toggleTraffic_" + key).click( function() {
            manageTrafficButton(getlocations_map[key], global_settings.trafficInfo[key], key)
          });
        }
        ...
        if (settings.bicycleinfo) {
          ...
          $("#getlocations_toggleBicycle_" + key).click( function() {
            manageBicycleButton(getlocations_map[key], global_settings.bicycleInfo[key], key)
          });
        }
        ...
        if (settings.transitinfo) {
          ...
          $("#getlocations_toggleTransit_" + key).click( function() {
            manageTransitButton(getlocations_map[key], global_settings.transitInfo[key], key)
          });
        }
        ...
        if (settings.panoramio_use && settings.panoramio_show) {
          ...
          $("#getlocations_togglePanoramio_" + key).click( function() {
            managePanoramioButton(getlocations_map[key], global_settings.panoramioLayer[key], key)
          });
        }
        // weather layer
        if (settings.weather_use && settings.weather_show) {
          ...
          if (settings.weather_cloud) {
            ...
            $("#getlocations_toggleCloud_" + key).click( function() {
              manageCloudButton(getlocations_map[key], global_settings.cloudLayer[key], key)
            });
          }
          else {
          ...
          $("#getlocations_toggleWeather_" + key).click( function() {
            manageWeatherButton(getlocations_map[key], global_settings.weatherLayer[key], global_settings.cloudLayer[key], key)
          });
        }
