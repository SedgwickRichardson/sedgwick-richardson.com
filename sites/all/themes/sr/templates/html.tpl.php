...
<!--[if (gte IE 9)|(gt IEMobile 7)]><!--><html <?php print $html_attributes . $rdf_namespaces; ?>><!--<![endif]-->
  ...
  <body class="<?php print $classes; ?>" <?php print $attributes; ?> onunload="">
    ...
    $count = $doc['#block-crumbs-breadcrumb ol li']->length;
    foreach ($doc['#block-crumbs-breadcrumb ol li'] as $key => $value) {
      $text = trim(preg_replace('/›/', '', pq($value)->text()));
      if ($count == 1) {
        $doc['#header-left']->after('<h2 id="page-title">' . $text . '</h2>');
        $doc['.region-breadcrumb']->remove();
      } else {
        // section why-sr has special design
        // show parent title only
        if (strpos($classes, 'section-why-sr') != false) {
          if ($key == 0) {
            $doc['#header-left']->after('<h2 id="page-title"><a href="/why-sr">' . $text . '</a></h2>');
          }
        } else {
          $link = pq($value)->find('a')->attr('href');
          if ($key == 0) {
            $doc['#header-left']->after('<h2 id="parent-title"><a href="' . $link . '">' . $text . '</a></h2>');
          } elseif ($key == 1) {
            $doc['#parent-title']->after('<h2 id="page-title">' . $text . '</h2>');
          }
        }
      }
      
    }
    $doc->find('.region-breadcrumb:eq(0)')->remove();
    print $doc;
    phpQuery::unloadDocuments();

//    print $page;
    ?>
    <span id="scrolltop">scroll to top</span>
    ...
    <script>
      ...
      ga('create', 'UA-22804685-10', 'sedgwick-richardson.com');
      ga('send', 'pageview');
    </script>
  </body>
</html>
